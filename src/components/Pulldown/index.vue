<template>
  <div class="toon-loadmore">
    <div class="toon-loadmore__content" :class="{ 'is-dropped': topDropped || bottomDropped}" :style="{ 'transform': 'translate3d(0, ' + translate + 'px, 0)' }" v-el:loadmore-content>
      <slot name="top">
        <div class="toon-loadmore__top" v-if="topMethod">
          <spinner v-if="topStatus === 'loading'" class="toon-loadmore__spinner" :size="20" type="fading-circle"></spinner>
          <span class="toon-loadmore-text">{{ topText }}</span>
        </div>
      </slot>
      <slot></slot>
      <slot name="bottom">
        <div class="toon-loadmore__bottom" v-if="bottomMethod">
          <spinner v-if="bottomStatus === 'loading'" class="toon-loadmore__spinner" :size="20" type="fading-circle"></spinner>
          <span class="toon-loadmore__text">{{ bottomText }}</span>
        </div>
      </slot>
    </div>
  </div>
